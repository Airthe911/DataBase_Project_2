...
def add_book(request):
    try:
        ...
        models.Store.objects.create(store_id=store_id, book_id=book_id, price=json.loads(book_info).get("price"), stock_level=stock_level)
        return JsonResponse({"status": 200, "message": "ok"})
    except BaseException as e:
        return JsonResponse({"status": 530, "message": "{}".format(str(e))})
...
def add_stock_level(request):
    try:
        ...
        if not models.Store.objects.filter(store_id=store_id, book_id=book_id).exists():
            return JsonResponse({"status": 922, "message": "新增库存中，store_id或book_id错误"})
