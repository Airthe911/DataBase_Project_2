...
def receipt(request):
    ...
    if not models.OrderInfo.objects.filter(order_id=order_id, user_id=user_id).exists():
        return JsonResponse({"status": 905, "message": "订单信息不匹配"})
    # step3 收货
    if not models.OrderInfo.objects.filter(order_id=order_id, user_id=user_id, state=2).exists():
        return JsonResponse({"status": 909, "message": "只有处于已发货、未收货的状态，才能进行本操作"})
...
def lookup(request):
    ...
    return JsonResponse({"status": 200, "message": str(result)})
...
def cancer(request):
    ...
    if state == 0:  # 订单尚未付款,只需返还数据
        ...
        if not models.User2Store.objects.filter(store_id=back_store_id).exists():
            return JsonResponse({"status": 919, "message": "店铺不存在"})
        ...
        return JsonResponse({"status": 917, "message": "已发货或已收货的订单无法取消"})
